...
### A 59.7MB dataset will appear.  From the explorer window, unzip the files desired, copy/paste them into the working directory.
###Files are X_test.txt, y_test.txt, features.txt, subject_test.txt
...
dim(features)
### features contains 561 observations and 2 variables. The variable names are V1 and V2

 

###Print out all the file.  
###features and x_test are both data frames.
### The V2 column of features has the variable names for the 561 variables of x_test.  We only want means and standard deviations.
### So locate which va riable name has "-mean" or "std" included on it.  Keep these variables, and later, rename them for clarity.
### Use dplyr package. Select the variables which are means and standard deviation


x_test_selected <- select(x_test, V1, V2, V3, V4, V5, V6, V41, V42, V43, V44, V45, V46, V81, V82, V83, V84, V85, V86, V121, V122, V123, V124, V125, V126, V161, 
V162, V163, V164, V165, V166, V201, V202, V214, V215, V227, V228, V240, V241, V253, V254, V266, V267, V268, V269, V270, V271,V294, V295, V296, V345, V346, V347, V348, V349, V350,
V241, V253, V254, V266, V267, V268, V269, V270, V271,V294, V295, V296, V345, V346, V347, V348, V349, V350, V373, V374,V375,
 V424, V425, V426, V427,V428, V429, V452, V453, V454, V503, V504, V513, V516, V517, V526, V529, V530, V539, V542, V543, V552)

### Rename the variables that were selected
 ...
 names(subject_test)
 ###Here, V1 is the subject.  Rename this column.
 subject_test <- rename(subject_test, subject= V1)
 names(subject_test)
 names(y_test)
 ###Here, V1 is the activity.  Rename this column.
...
### y_test consists of a single column of integers from 1 to 6.  There are 2947 rows, so the activities can be matched with the observations of X_test_renamed.
###First replace the integers 1 to 6 with the corresponding activity names.
...
#### subject_test and x_test have the same number of elements.  c-bind them to get an identifier for each entry.
#

  ### Creating a dataframe for training data.  From the explorer window, unzip the files desired, copy/paste them into the working directory.
  ###Files are X_train.txt, y_train.txt, features.txt, subject_train.txt
  ### Read the files into R.  
    ...
    features <- read.table("features.txt", header= FALSE)
     ...
     ### The V2 column of features has the variable names for the 561 variables of x_train.  We only want means and standard deviations.
     ### So locate which va riable name has "-mean" or "std" included on it.  Keep these variables, and later, rename them for clarity.
     ...
     x_train_selected <- select(x_train, V1:V6, V41:V46, V81:V86, V121:V126, V161:V166, 
                                V201, V202, V214, V215, V227, V228, V240, V241, V253, V254, V266, V267, V268, V269, V270, V271,V294, V295, V296, V345:V350, 
                               V373, V374,V375,
     ...
     ### Look at subject_train
     head(subject_train)
### Rename the V1 column in subject_train
     subject_train <- rename(subject_train, subject= V1)
    
### Rename the V1 column in y_train
     y_train <- rename(y_train, activity= V1)
### Create the dataframe traindata using the cbind function.
     traindata <- cbind(subject_train, y_train, x_train_renamed)


names(traindata)
### Add a column to indicate training.
...
### Before rbinding traindata and testdata, make sure that the columns match.
...
###5 STANDING
###6 LAYING
activity_type <- function(measurement, activity){
  if (activity==1) {x="walking"}
  if (activity ==2) {x="walking upstairs"}
  if (activity ==3) {x = "walking downstairs"}
  if (activity ==4) {x = "sitting"}
  if (activity ==5) {x= "standing"}
  if (activity ==6)  {x= "laying"}
x}
measure_dat <- mutate(measurement, act =x)
...
laying <- filter(measurements, activity==6)
  ...
  laying<- select( laying, -activity)   
  ### rbind the separate Activity sets to get the total
  measurement_dat<- rbind(walking, walking_upstairs, walking_downstairs, sitting, standing, laying)
  names(measurement_dat)
  
names(measurement_dat)
View(measurement_dat)
### Rename the Activity column
measurement_dat <- rename(measurement_dat, activity=Activity)
library(dplyr)
### Group the data by activity and subject
### Make sure that measurement_dat is a dataframe
dat <- tbl_df(measurement_dat)
### Group dat by activity and subject
by_activity_subject <-group_by (dat, activity, subject)
###Look at by_activity_subject
by_activity_subject
### There are 180 activity_group level.  180 = 6 * 30, where 6 is the number of activities and 30 is the number of subjects.
###Use the summarize function to get means. Call the dataset data1.
